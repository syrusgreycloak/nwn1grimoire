...
void main() {
    ...
    //*:**********************************************
    int     iIncrease       = (spInfo.iSpellID-298)*3;

    int     iConMod         = GetAbilityModifier(ABILITY_CONSTITUTION);

    if(spInfo.iSpellID==SPELLABILITY_RAGE_3) {
        iConMod++;
        if(iConMod<=0) iConMod = 1;
    } else {
        iConMod = (((GetAbilityScore(oCaster, ABILITY_CONSTITUTION) - 10) + iIncrease)/2) + iConMod;
    }

    float   fDuration       = GRGetDuration(iConMod);
    ...
    //*:* Effects
    //*:**********************************************
    effect eDex = EffectAbilityIncrease(ABILITY_DEXTERITY, iIncrease);
    effect eCon = EffectAbilityIncrease(ABILITY_CONSTITUTION, iIncrease);
