...
void main() {
    ...
    effect eDmg     = EffectDamageIncrease(GRGetDamageBonusValue(iBonus));
    ...
    while(GetIsObjectValid(spInfo.oTarget)) {
        if(GRGetIsSpellTarget(spInfo.oTarget, SPELL_TARGET_ALLALLIES, oCaster, TRUE)) {
            SignalEvent(spInfo.oTarget, EventSpellCastAt(oCaster, SPELL_GR_GOOD_HOPE));
            if(!GetHasSpellEffect(SPELL_CRUSHING_DESPAIR, spInfo.oTarget)) {
                GRApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, spInfo.oTarget);
                GRApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, spInfo.oTarget, fDuration);
            } else {
                GRRemoveSpellEffects(SPELL_CRUSHING_DESPAIR, spInfo.oTarget);
            }
        }
