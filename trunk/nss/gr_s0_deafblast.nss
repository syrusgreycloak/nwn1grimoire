...
void main() {
    ...
    effect eImp  = EffectVisualEffect(GRGetAlignmentImpactVisual(oCaster, fRange));
    ...
    while(GetIsObjectValid(spInfo.oTarget)) {
        if(GRGetIsSpellTarget(spInfo.oTarget, SPELL_TARGET_STANDARDHOSTILE, oCaster, NO_CASTER)) {
            SignalEvent(spInfo.oTarget, EventSpellCastAt(oCaster, SPELL_GR_DEAFENING_BLAST));
            if(!GRGetSpellResisted(oCaster, spInfo.oTarget)) {
                ...
                if(!GRGetSaveResult(SAVING_THROW_FORT, spInfo.oTarget, spInfo.iDC, SAVING_THROW_TYPE_SONIC, oCaster, fDelay)) {
                    DelayCommand(fDelay, GRApplyEffectToObject(DURATION_TYPE_PERMANENT, eDeaf, spInfo.oTarget));
                } else {
                    DelayCommand(fDelay, GRApplyEffectToObject(DURATION_TYPE_TEMPORARY, eDeaf, spInfo.oTarget, fDuration));
                }
