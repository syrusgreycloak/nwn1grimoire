...
//*:*  GR_S0_HOLYSWRD.NSS
//*:**************************************************************************
//*:* Holy Sword (X2_S0_HolySwrd) Copyright (c) 2001 Bioware Corp.
...
void main() {
    ...
    if(GetIsObjectValid(oMyWeapon)) {
        SignalEvent(oMyWeapon, EventSpellCastAt(oCaster, GetSpellId(), FALSE));
        if(fDuration>0.0) {
            ...
            GRAddHolyAvengerEffectToWeapon(oMyWeapon, fDuration);
        }
        TLVFXPillar(VFX_IMP_GOOD_HELP, GetLocation(GetSpellTargetObject()), 4, 0.0f, 6.0f);
        DelayCommand(1.0f, GRApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_IMP_SUPER_HEROISM),
            GetLocation(spInfo.oTarget)));
