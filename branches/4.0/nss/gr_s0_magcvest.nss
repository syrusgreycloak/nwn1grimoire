...
//*:*  GR_S0_MAGCVEST.NSS
//*:**************************************************************************
//*:* Magic Vestment (X2_S0_MagcVest) Copyright (c) 2001 Bioware Corp.
...
void main() {
    ...
    //*:* int     iDieType          = 0;
    //*:* int     iNumDice          = 0;
    int     iBonus            = MinInt(5, spInfo.iCasterLevel/4);
    //*:* int     iDamage           = 0;
    //*:* int     iSecDamage        = 0;
    ...
    if(GetIsObjectValid(oMyArmor)) {
        ...
        if(fDuration>0.0) {
            DelayCommand(1.3f, GRApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, GetItemPossessor(oMyArmor)));
            GRApplyEffectToObject(DURATION_TYPE_TEMPORARY, eDur, GetItemPossessor(oMyArmor), fDuration);
            GRAddACBonusToArmor(oMyArmor, fDuration, iBonus);
        }
        return;
    } else {
