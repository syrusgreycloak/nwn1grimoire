...
//*:*  GR_S1_HORRAPPRA.NSS
//*:**************************************************************************
//*:* Aura of Troglodyte Stench On Enter (nw_s1_trogstinkA.nss) Copyright (c) 2004 Bioware Corp.
//*:* Created By: Craig Welburn  Created On: Nov 6, 2004
//*:* 3.5 Monster Manual (p. 246)
//*:**************************************************************************
...
void DoStrengthDrain(int iDrainAmount, object oTarget) {
    ...
    GRApplyEffectToObject(DURATION_TYPE_TEMPORARY, eSTRDrain, oTarget, GRGetDuration(10));
}
...
void main() {
    ...
    //*:* Targets can only be affected once in a 24hr period by the same Troglodyte.  We'll cheat and use
    //*:* "same day", so there could be a minor technicality if fighting around midnight
    if(GetLocalInt(spInfo.oTarget, "GR_TROG_STENCH_DAY"+ObjectToString(oCaster))==GetCalendarDay()) {
        return; // already affected
    } else {
        SetLocalInt(spInfo.oTarget, "GR_TROG_STENCH_DAY"+ObjectToString(oCaster), GetCalendarDay());
    }
    ...
    if(spInfo.oTarget!=oCaster) {
        if(GetIsEnemy(spInfo.oTarget, oCaster) && !GetIsReactionTypeFriendly(spInfo.oTarget, oCaster)) {
            if(!GRGetSaveResult(SAVING_THROW_FORT, spInfo.oTarget, spInfo.iDC, SAVING_THROW_TYPE_NONE, oCaster)) {
                ...
                //*:* We will use a delayed function to disconnect from the spell id, so
