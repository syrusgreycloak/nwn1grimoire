...
void main() {
    ...
    effect ePara    = SupernaturalEffect(EffectParalyze());
    effect eDur     = EffectVisualEffect(VFX_DUR_PARALYZE_HOLD);

    effect eParaLink= EffectLinkEffects(ePara, eDur);
    ...
    if(TouchAttackMelee(spInfo.oTarget)) {
        if(!GRGetSpellResisted(oCaster, spInfo.oTarget)) {
            ...
            if(!GetIsImmune(spInfo.oTarget, IMMUNITY_TYPE_FEAR, oCaster)) {
                GRApplyEffectToObject(DURATION_TYPE_TEMPORARY, eShaken, spInfo.oTarget, GRGetDuration(1));
            }
            if(!GRGetSaveResult(SAVING_THROW_FORT, spInfo.oTarget, spInfo.iDC, SAVING_THROW_TYPE_NEGATIVE)) {
                GRApplyEffectToObject(DURATION_TYPE_PERMANENT, eParaLink, spInfo.oTarget);
            }
        }
    }
