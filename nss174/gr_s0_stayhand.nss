...
void main() {
    ...
    //*:* Apply effects
    //*:**********************************************
    if(GRGetIsHumanoid(spInfo.oTarget)) {
        SignalEvent(spInfo.oTarget, EventSpellCastAt(oCaster, SPELL_GR_STAY_THE_HAND, TRUE));
        if(!GRGetSpellResisted(oCaster, spInfo.oTarget)) {
            if(!GRGetSaveResult(SAVING_THROW_WILL, spInfo.oTarget, spInfo.iDC, SAVING_THROW_TYPE_MIND_SPELLS)) {
                ...
                SetIsTemporaryFriend(spInfo.oTarget, oCaster, TRUE, fDuration);
            }
        }
    }
