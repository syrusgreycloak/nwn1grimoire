...
void main() {
    ...
    float   fDuration       = GRGetSpellDuration(spInfo)*2.0;
    float   fDelay          = 0.0f;
    ...
    //*:**********************************************
    int     iBlindDuration  = GRGetMetamagicAdjustedDamage(6, 2, spInfo.iMetamagic);
    if(GRGetMetamagicUsed(spInfo.iMetamagic, METAMAGIC_EXTEND)) iBlindDuration *= 2;
    ...
    if(GetIsObjectValid(spInfo.oTarget)) {
        ...
        if(!GRGetSpellResisted(oCaster, spInfo.oTarget)) {
            ...
            if(WillSave(spInfo.oTarget, spInfo.iDC, SAVING_THROW_TYPE_FEAR)==0) {
                GRApplyEffectToObject(DURATION_TYPE_TEMPORARY, eFrightened, spInfo.oTarget, fDuration);
                SetLocalInt(spInfo.oTarget,"GR_BSTM_IS_FRIGHT",TRUE);
                if(GetHitDice(spInfo.oTarget)<=8) {
                    ...
                    SetLocalInt(spInfo.oTarget,"GR_BSTM_IS_PANIC",TRUE);
                }
            }
        }
    }
