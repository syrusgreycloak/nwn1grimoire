...
void main() {
    ...
    location lBehindLocation= GetBehindLocation(spInfo.oTarget);
    ...
    if(!GRGetIsSpellTarget(spInfo.oTarget, SPELL_TARGET_ALLALLIES, oCaster)) {
        if(!GRGetSpellResisted(oCaster, spInfo.oTarget)) {
            if(!GRGetSaveResult(SAVING_THROW_WILL, spInfo.oTarget, spInfo.iDC)) {
                ...
                DelayCommand(0.2, AssignCommand(spInfo.oTarget, JumpToLocation(lBehindLocation)));
            }
        }
    }
