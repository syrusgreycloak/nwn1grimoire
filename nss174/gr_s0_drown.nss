...
//*:*  GR_S0_DROWN.NSS
//*:**************************************************************************
//*:* Drown [X0_S0_Drown.nss] Copyright (c) 2002 Bioware Corp.
//*:* Created By: Brent  Created On: July 26 2002
//*:* Spell Compendium (p. 74)
//*:**************************************************************************
//*:* Mass Drown
...
void main() {
    ...
    int     bMultiTarget    = (spInfo.iSpellID==SPELL_MASS_DROWN);
    ...
    if(GetIsObjectValid(spInfo.oTarget)) {
        do {
            if(!bMultiTarget || GRGetIsSpellTarget(spInfo.oTarget, SPELL_TARGET_STANDARDHOSTILE, oCaster, NO_CASTER)) {
                SignalEvent(spInfo.oTarget, EventSpellCastAt(oCaster, spInfo.iSpellID));
                if(!GRGetSpellResisted(oCaster, spInfo.oTarget)) {
                    if(GRGetIsLiving(spInfo.oTarget) && GRGetRacialType(spInfo.oTarget)!=RACIAL_TYPE_ELEMENTAL) {
                        if(!GRGetSaveResult(SAVING_THROW_FORT, spInfo.oTarget, spInfo.iDC)) {
                            iDamage = FloatToInt(GetCurrentHitPoints(spInfo.oTarget)*0.9);
