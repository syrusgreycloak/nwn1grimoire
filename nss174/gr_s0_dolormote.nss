...
void main() {
    ...
    effect eAOE = EffectAreaOfEffect(AOE_PER_DOLOROUS_MOTES);
    ...
    object oAOE = GRGetAOEAtLocation(spInfo.lTarget, AOE_TYPE_DOLOROUS_MOTES, oCaster);
    GRSetAOESpellId(spInfo.iSpellID, oAOE);
    GRSetSpellInfo(spInfo, oAOE);

    //*:**********************************************
    //*:* Sacrifice spell component
    //*:**********************************************
    int iWisDamage  = GRGetMetamagicAdjustedDamage(3, 1, METAMAGIC_NONE, 0);
    effect eWisDec  = EffectAbilityDecrease(ABILITY_WISDOM, iWisDamage);
    effect eVis     = EffectVisualEffect(VFX_IMP_REDUCE_ABILITY_SCORE);
    SignalEvent(oCaster, EventSpellCastAt(oCaster, SPELL_GR_DOLOROUS_MOTES, FALSE));
    GRApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oCaster);
    GRApplyEffectToObject(DURATION_TYPE_PERMANENT, eWisDec, oCaster);
